...
export function EditorPage() {
  ...
  // Mark as unsaved when nodes change after initial load
  useEffect(() => {
  ...
  const onConnect = useCallback(
    (params: Connection) =>
      setEdges((eds) =>
        addEdge(
          { ...params, animated: true, style: { stroke: '#3b82f6' } } as Edge,
          eds
        )
      ),
    [setEdges]
  ...
  function buildPipelineFromNodes(currentNodes: Node[]): Pipeline {
    ...
    console.log('[save] sources:', sources, 'transforms:', transforms, 'targets:', targets);
  ...
  async function handleSave(currentNodes = nodes): Promise<Pipeline | null> {
  ...
  return (
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', background: '#0a0f1e', fontFamily: 'monospace' }}>
      ...
      <div style={{
        ...
        {saveStatus === 'unsaved' && (
          <span style={{ fontSize: 11, color: '#f59e0b' }}>● nesalvat</span>
        )}
        {saveStatus === 'saved' && (
          <span style={{ fontSize: 11, color: '#22c55e' }}>✓ salvat</span>
        )}
        <div style={{ flex: 1 }} />
        <button
          onClick={() => handleSave()}
          ...
          style={{
            padding: '6px 16px', background: saveStatus === 'unsaved' ? '#0284c7' : '#1e293b',
            color: '#fff', border: 'none', borderRadius: 6, fontWeight: 600,
            cursor: saving ? 'wait' : 'pointer', fontSize: 13, opacity: saving ? 0.7 : 1,
          }}
        >
          {saving ? '...' : '💾 Salveaza'}
        </button>
        <button
          ...
          style={{ padding: '6px 18px', background: '#16a34a', color: '#fff', border: 'none', borderRadius: 6, fontWeight: 700, cursor: 'pointer', fontSize: 13 }}
        >
          ▶ Executa
        </button>
      </div>
